...
This is the new system that equivalent to seaon 1's armband. Only three options are worth considering for Skyward: <span style="color:green">tr_MHJL</span>, <span style="color:blue">tr_JM</span>, <span style="color:red">Illusion Arrow</span>. Each of these options are timegated, as the season progress they unlock from left to right, top to bottom.
...
Use tr_MHJL at the start of the season. When tr_JM unlocks, either keep using tr_MHJL or switch to tr_JM. Switch to Illusion Arrow when it unlocks for final build.

Illusion Arrow's main effect:

When using damage type expertise skill (Falcon, Instant Edge, Sharp Impact, Galeform) summons Illusion Arrow, this effect can trigger once every 7 seconds. Each time this is triggered the next summon will increase arrow count by 1, up to a max of 8 arrows (later on in the tree there's a branch that increases the max up to 10). Each time special attack (Skyfall) is used, it reduces the cooldown by 0.6 second. As long as you are continously performing rotation, you will be maximizing the effect of Illusion Arrow.


For Illusion Arrow branches select the following :
- Reduce CD (left)
...
- tr_MHJL This is the first one that gets unlocked, good to use for early game
- Illusion Arrow This gets unlocked later, and it will be the best S2Armband for Skyward
- tr_JM This used to be a viable option too but it has since been nerfed, and on top of that it's usually used by tank, which allows everyone in the party to get partial benefit, so when a dps also use it its value is diminished on the dps. This should only be considered when Illusion Arrow is still locked behind timegate.

### Illusion Factor

In the S2Armband we select 2 class specific Illusion Factors and 3 common attack Illusion Factors.
...
- Gear Substats:
    ...
    - For the Crit / Haste pieces, it doesn't matter which stat gets high roll, you can adjust with 3rd reforge roll and imprints to reach your desire crit and haste %.
...
| Lv120 Early Game | Typhoon (6 Sharp SI)<br>(Less Than 50% Crit) | Non-Typhoon (3 Sharp SI)<br>(More Than 50% Crit) |
|:-:|:-:|:-:|
| <img src="./assets/s2tornado.png" /> | <img src="./assets/s2typhoon.png" /> | <img src="./assets/s2nontyphoon.png" /> | |
...
The skill 7997 is Illusion Arrow.
